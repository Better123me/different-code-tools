...
def GetComment2(comment_oid):
    ...
    for oid in comment_oid:
        ...
        for page in range(5):
            ...
            tmp_comment.pop()
            tmp_comment.pop()
            content_list.append(tmp_comment)
        data = pd.DataFrame(content_list)
        data.to_excel('评论区' + str(cnt + 1) + ".xlsx", sheet_name='Sheet1')
        print(f"评论区{cnt + 1}号爬取完成")
        cnt += 1